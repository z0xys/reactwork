    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (response.ok) {
                ...
                console.log('New Division added:', result);
                navigate('/app/division/view');
            } else {
                console.error('Error adding Division:', response.status);
            }
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <div>
                ...
                {errors.divisionName?.type === 'required' && (
                    <span>This field is required</span>
                )}
                {errors.divisionName?.type === 'maxLength' && (
                    <span>Name cannot exceed 20 characters</span>
                )}
                <div className="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                    <input
                        type="text"
                        {...register('divisionName', { required: true, maxLength: 20 })}
